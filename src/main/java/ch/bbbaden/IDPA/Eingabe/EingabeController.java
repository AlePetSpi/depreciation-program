 ...
 * @author Alexander
 */
public class EingabeController implements ChangeListener, Initializable {
    ...
    private Label label;
    @FXML
    private JFXTextField anschaffungswert;
    @FXML
    ...
    private void abschreibunganzeigen(ActionEvent event) throws IOException {
        if (datenuebertragung() == false) {
            ...
            loader.setLocation(getClass().getResource("/fxml/Ausgabe.fxml"));
            loader.load();
    ...
    private boolean fehlerEingabeRestbetrag() {
        if (radiolinear.isSelected()) {
            try {
                anschaff = Integer.parseInt(this.anschaffungswert.getText());
    ...
    private boolean fehlerEingabeAnschaffungswert() {
        int bet;
        try {
            bet = Integer.parseInt(this.anschaffungswert.getText());
            anschaff = bet;
